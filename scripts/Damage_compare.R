...
#REDO OF SOME OF THE STEPS ABOVE:
...
ggplot(bu_dmg_all, aes(x = site)) +
  geom_bar(aes(fill = status), position = "dodge")

ggplot(data = damage10_BU) + 
   geom_bar(mapping = aes(x = site ))
#> ggplot(data = bu_dmg_all) + 
...
ggplot(data=dmg_bu_AD) +
  geom_bar(mapping=aes(x=status))

ggplot(data = dmg_bu_AD) + 
  geom_bar(mapping = aes(x = status, fill=b_u,y = ..prop..))

ggplot(data = dmg_bu_AD) + 
  geom_bar(mapping = aes(x = status, fill=b_u,y = ..prop..,group=1))
